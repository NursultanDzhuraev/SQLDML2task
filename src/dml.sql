...
select count(id) as "Chevrolet" from cars where brand ='Chevrolet';
...
select sum(price) as "Mercedes-Benz" from cars where brand = 'Mercedes-Benz';
...
select * from cars where year_of_issue>2005 or year_of_issue<1995;
-- #18 Бир тустогу саны эн коп болгон машиналарды чыгаргыла
select * from cars
where color=(
    select cars.color from cars group by color order by count(*) desc limit 1);